...
// IMPORT: Yahan apne QuestionForm ka path theek kar lijiye ga agar folder alag hai
...
export default function QuestionsPage() {
  ...
  // 1. FETCH QUESTIONS (Isay useEffect se bahar nikal liya taake refresh kar sakein)
  ...
  const handleAddNewClick = () => {
    setSelectedQuestion(null); // Khali form ke liye null
    setIsModalOpen(true);
  };

  const handleEditClick = (question: Question) => {
    setSelectedQuestion(question); // Purana data form mein bhejne ke liye set kiya
    setIsModalOpen(true);
  };
  ...
  const handleFormSuccess = () => {
    handleCloseModal();
    fetchQuestions(); // Save/Update ke baad table ko auto-refresh karna
  };

  const handleDelete = async (id: number) => {
    if (window.confirm("Are you sure you want to permanently delete this question?")) {
      try {
        const response = await fetch(`http://localhost:5064/api/admin/questions/${id}`, {
          method: 'DELETE',
        });
        if (response.ok) {
          fetchQuestions(); // Delete hone ke baad table refresh
        } else {
  ...
  return (
    <div className="p-8 space-y-8 relative">
                    ...
                    <td className="p-6">
                      <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-all">
                        ...
                        <button 
                          ...
                          className="p-2 text-slate-400 hover:text-blue-600 bg-white rounded-lg border border-slate-100 transition-colors"
                        >
                          <Edit3 size={14} />
                        </button>
                        ...
                        <button 
                          ...
                          className="p-2 text-slate-400 hover:text-rose-600 bg-white rounded-lg border border-slate-100 transition-colors"
                        >
                          <Trash2 size={14} />
                        </button>
                      </div>
      ...
      {/* MODAL POPUP BACKGROUND (Yahan QuestionForm render hoga) */}
      {isModalOpen && (
